...
Morph.prototype.contextMenu = function(){nop()};
WorldMorph.prototype.contextMenu = function(){nop()};
...
function dedup(data, attributes){
    var temp = [];
    data = data.sort(function(a, b){return full_cmp(a, b, attributes);});
    var last = data[0];
    temp.push(data[0]);
    for (var i=0; i < data.length; i++){
    ...
    if (result) {
        ...
        if(result != null){
            ...
            if(data.length > 0){
                ...
                return {"table_name": "table_result", "data": data, 'name': full['name'], 'attributes': full['attributes']};}}
            else{
                return null;
        }
    }
...
query_operators['Project'] = function (){
    ...
    if (result) {
        ...
        if(result != null){
            ...
            result = result['data'];
            var data = [], temp = [];
            operation = operation.filter(function(op){return result[0].hasOwnProperty(op)});
...
query_operators['GroupBy'] = function (){
    ...
    if (result) {
        ...
        if(result != null){
            ...
            var inputs = this.children.filter(function(child){return child instanceof InputMorph});
            var temp1 = detect(inputs[0].children, function(child){return child instanceof StringMorph});
            var temp2 = detect(inputs[1].children, function(child){return child instanceof StringMorph});
            ...
            var query_result = [];
            result.sort(function(a, b){return full_cmp(a, b, groupby);});
            ...
            for(i=0; i < result.length; i++){
                ...
                for(var ak=0; ak<aggregation_keys.length; ak++){
                    ...
                    if(!current_group_result.hasOwnProperty(aggregation_keys[ak])){
                        var initial = null;
                        if (op == 'sum' || op == 'count' || op == 'distinct-count'){initial = 0;}
                    ...
                    switch(op){
                        ...
                        case 'MAX':
                            ...
                            else{current_group_result[aggregation_keys[ak]] = result[i][val]}
                            break;
                        case 'MIN':
                            if(current_group_result[aggregation_keys[ak]]){
                                if(current_group_result[aggregation_keys[ak]] > result[i][val]){
...
query_operators['NaturalJoin'] = function (){
    var children_data = this.nextBlocks(),
        left_data = detect(children_data, function(child){return child.blockPosition === 'bottom_left'}),
        right_data = detect(children_data, function(child){return child.blockPosition === 'bottom_right'});
    if(children_data.length === 2){
        var data = [];
        if(left_data && right_data){
            left_data = left_data.get_query(); right_data = right_data.get_query();
            if(left_data != null && right_data != null){
                var left_attrs = left_data['attributes'], right_attrs = right_data['attributes'];
                left_data = left_data['data']; right_data = right_data['data'];
                var keys;
...
//                $.each(right_data[0], function(k, v){right_keys.push(k)});
                keys = left_attrs.filter(function(k){return right_attrs.indexOf(k) != -1}); //common attributes
                console.log(keys);
                if(keys.length != 0) {
    ...
    if(children_data.length === 2){
        ...
        if(left_data && right_data){
            ...
            if(left_data != null && right_data != null){
                var left_attrs = left_data['attributes'], right_attrs = right_data['attributes'];
                left_data = left_data['data']; right_data = right_data['data'];
                var matches = right_attrs.filter(function(k){return left_attrs.indexOf(k) != -1});
                if (matches == 0){
                    var attrs = left_attrs.concat(right_attrs);
    ...
    if(children_data.length === 2){
        ...
        if(left_data && right_data){
            ...
            if(left_data != null && right_data != null){
                ...
                keys = left_keys.filter(function(k){return right_keys.indexOf(k) != -1});
                if(keys.length == 0){
                    for (var i=0; i<left_data.length; i++){
...
query_operators['Union'] = function (){
    ...
    if(children_data.length === 2){
        ...
        if(left_data && right_data){
            ...
            if(left_data != null && right_data != null){
                var left_attrs = left_data['attributes'], right_attrs = right_data['attributes'];
                var matches = left_attrs.filter(function(attr){return right_attrs.indexOf(attr) == -1});
                if (matches.length == 0){
                    left_data = left_data['data']; right_data = right_data['data'];
                    var temp = left_data.concat(right_data);
                    temp = temp.sort(function(a, b){return full_cmp(a, b, left_attrs);});
                    var last = temp[0];
                    data.push(temp[0]);
        ...
        if(left_data && right_data){
            ...
            if(left_data != null && right_data != null){
                ...
                if (matches.length == 0 && left_attrs.length == right_attrs.length){
                    left_data = left_data['data']; right_data = right_data['data'];
                    left_data.sort(function(a, b){return full_cmp(a, b, left_attrs)});
                    right_data.sort(function(a, b){return full_cmp(a, b, left_attrs)});
...
relalg_operators['NaturalJoin'] = function (){
    ...
    if(children_data.length === 2){
        if(left_data && right_data){
            var viewIsChildren = true;
            left_data = left_data.get_relalg(viewIsChildren); right_data = right_data.get_relalg(viewIsChildren);
            if(left_data != null && right_data != null){
                    left_data = left_data['relalg']; right_data = right_data['relalg'];
...
relalg_operators['Intersection'] = function (){
    ...
    if(children_data.length === 2){
        if(left_data && right_data){
            var viewIsChildren = true;
            left_data = left_data.get_relalg(); right_data = right_data.get_relalg();
            if(left_data != null && right_data != null){
                    left_data = left_data['relalg']; right_data = right_data['relalg'];
...
function RunQueryMorph(){this.init()}
...
RunQueryMorph.prototype.mouseClickLeft = function(){
    ...
    var roots = script.children.filter(function(child){return child instanceof BaseBlockMorph;});
    if (roots.length === 1){
...
function ZoomMorph(zoom_type){this.init(zoom_type)}
...
function mouse_wait(){document.getElementById('body').style.cursor='wait'}
function mouse_auto(){document.getElementById('body').style.cursor='auto'}

var ScriptMorph;
ScriptMorph.prototype = new FrameMorph();
ScriptMorph.prototype.constructor = ScriptMorph;
ScriptMorph.uber = FrameMorph.prototype;
function ScriptMorph(owner) {this.init(owner);}
...
ScriptMorph.prototype.queryUpdate = function(){
    ...
    var run_query = function() {
        ...
        if (roots.length === 1) {
            ...
            tableCreate("table_result", result);
        } else {
            tableCreate('table_result', null);
            textbox.setText("Relation Algebra Expression:");
        }
        if (roots.length < 1){
            tableCreate('table_data', null);
    ...
    this.nextSteps([mouse_wait, function(){setTimeout(run_query, 1)}, mouse_auto]);
};
...
function BaseBlockMorph() {this.init();}
...
BaseBlockMorph.prototype.attachPoints = function(){return [];};
BaseBlockMorph.prototype.allAttachTargets = function (new_parent) {
    ...
    top_blocks.forEach(function(block){block.forAllChildren(function(child){
        if (child.attachPoints){child.attachPoints().forEach(function(attach_point){
...
BaseBlockMorph.prototype.mouseClickLeft = function(){
    ...
    var gui = this.parentThatIsA(GuiMorph);
    if (lastSelectedNode){
        strings = lastSelectedNode.children.filter(function(child){return child instanceof StringMorph});
        for (i=0; i < strings.length; i++){
            title = strings[i];
            title.color = 'white';
            title.drawNew();
    ...
    strings = this.children.filter(function(morph){return morph instanceof StringMorph});
    if (strings.length > 0) {
        for (i = 0; i < strings.length; i++) {
            title = strings[i];
            if (!(this.isTemplate)) {
...
BaseBlockMorph.prototype.getChildBlocks = function(){
    return this.children.filter(function(child){return child instanceof BaseBlockMorph});};
BaseBlockMorph.prototype.getInputMorph = function(){
    return detect(this.children, function(child){return child instanceof InputMorph;});
...
BaseBlockMorph.prototype.getInputText = function(){
    var inputs = this.children.filter(function(child){return child instanceof InputMorph});
    if (inputs.length == 1){
        return detect(inputs[0].children, function(child){return child instanceof StringMorph}).text;
    }
    else{
        var group = detect(inputs[0].children, function(child){return child instanceof StringMorph});
        var out = detect(inputs[1].children, function(child){return child instanceof StringMorph});
...
function OperatorMorph(op_name, query, input, color){this.init(op_name, query, input, color);}
...
OperatorMorph.prototype.attachPoints = function(){
    ...
    if(this.children.filter(function(child){return child instanceof BaseBlockMorph}).length === 0){
...
function OperatorGroupByMorph(op_name, query, input, color, params){this.init(op_name, query, input, color, params);}
...
//    this.rect_width = group.width()+30;
    if (operation.width()+30 > this.rect_width){this.rect_width = operation.width()+30;}
...
//    var input_field = detect(input.children, function(child){return child instanceof StringMorph});
    ...
    if (inputs[1].width()+30 > this.rect_width){this.rect_width = inputs[1].width()+30+55;}
    this.setExtent(new Point(this.radius*2+this.rect_width+this.gap_width+this.bwidth, this.radius*5));
    var script = this.parentThatIsA(ScriptMorph);
    if (script){
        script.adjustBounds();
...
function OperatorRenameMorph(op_name, query, input, color, params){this.init(op_name, query, input, color, params);}
...
//    this.rect_width = inputs[0].right();
    ...
    if (inputs[1].width()+30 > this.rect_width){this.rect_width = inputs[1].width()+30+24;}
    this.setExtent(new Point(this.radius*2+this.rect_width+this.gap_width+this.bwidth, this.radius*5));
    var script = this.parentThatIsA(ScriptMorph);
    if (script){
        script.adjustBounds()}
...
function OperatorJoinMorph(name, color, query){this.init(name, color, query)}
...
OperatorJoinMorph.prototype.attachPoints = function(){
    ...
    if(this.children.filter(function(child){
        return child instanceof BaseBlockMorph}).filter(function(child){return child.blockPosition === 'bottom_left'}).length === 0){
            points.push({"element": this, "point": new Point(this.left()+25+width, this.top()+100), "loc": "bottom_left"});}
    if(this.children.filter(function(child){
        return child instanceof BaseBlockMorph}).filter(function(child){return child.blockPosition === 'bottom_right'}).length === 0){
            points.push({"element": this, "point": new Point(this.left()+175-width, this.top()+100), "loc": "bottom_right"});}
    return points;};
...
function OperatorUnionMorph(name, color, query){this.init(name, color, query)}
OperatorUnionMorph.prototype.init = function(name, color, query){
    ...
    this.get_children = children_operators[query];
    var fontsize = 12;
    if (name == 'Difference'){fontsize=10}
...
function ThetaJoinInputMorph() {this.init();}
...
function OperatorThetaJoinMorph(name, params){this.init(name, params)}
...
OperatorThetaJoinMorph.prototype.ctrl = function(aChar){alert(aChar)};
OperatorThetaJoinMorph.prototype.snap = function(){
    var target = this.closestAttachTarget(), script = this.parentThatIsA(ScriptMorph);//, next, offset_y, affected;
    script.clearDropHistory();
    script.lastDroppedBlock = this;
...
function DataSetBlockMorph(data, name, queryParent){this.init(data, name, queryParent)}
...
function InputMorph(text, label){this.init(text, label)}
...
InputMorph.prototype.arrow = function(){
    return detect(this.children, function(child){return child instanceof ArrowMorph});
};
InputMorph.prototype.getText = function(){
    return detect(this.children, function(child){return child instanceof StringMorph});
};
...
InputMorph.prototype.reactToEdit = function(){this.getText().clearSelection()};
...
function SettingsButtonMorph(){this.init()}
