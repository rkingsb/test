...
ScriptMorph.prototype.queryUpdate = function () {
    ...
    var run_query = function () {
        ...
        if (roots.length === 1) {
            ...
            var attributeAcceptedList = new Array("SID", "FNAME", "LNAME"); // Austin S., the list of accepted attributes for the "Attr" field for SELECT blocks
            if(query) // Austin S., determining errors, check if there is an output of relational algebra
            {
                var relStr = query['relalg'];
                if(relStr != "Relational Algebra Expression:") // Austin S., if it outputs rel algebra, make sure it's not the default text
                {
                    var relArray = relStr.split(" ");
                    for(index = 0; index < relArray.length; index++)
                    {
                        if(relArray[index] == "")
                        {
                    ...
                    if(relStr.includes("SELECT") && !relStr.includes("Attr OP Value")) // Austin S., handle Selects
                    {
                        var selects = new Array();
                        for(i = 0; i < relArray.length; i++)
                        {
                            if(relArray[i].includes("SELECT"))
                            {
                                var selectBlock = new Array();
                                var attr = relArray[i+1];
                                if(!checkAttribute(attr))
                                {
                                    update_content("ERROR: Misspelled Column" + "," + attr);
                                }
                                var oP = relArray[i+2];
                                var value = relArray[i+3];
                                var cond = relArray[i+4];
                                selectBlock.push(attr);
                                selectBlock.push(oP);
                                selectBlock.push(value);
                                selectBlock.push(cond);
                                selects.push(selectBlock);
                                i = i + 4;

                                function checkAttribute(currentAttribute) // Austin S., function that checks if current attribute is part of the accepted attribute list
                                {
                                    return attributeAcceptedList.some(compareAttrValue);
                                    function compareAttrValue(value)
                                    {
                                        return value == currentAttribute;
									}
								}
                            }
                        }
                    }
                    if(relStr.includes("PROJECT") && !relStr.includes("Attrbs")) // Austin S., handle Projects
                    {

                    }
                    if(relStr.includes("GROUP") && !relStr.includes("Attrs") && !relStr.includes("function(Attr)")) // Austin S., handle Group Bys
                    {

                    }
                    if(relStr.includes("Rename Order->") && !relStr.includes("Attr") && !relStr.includes("NewRel")) // Austin S., handle Renames
                    {
